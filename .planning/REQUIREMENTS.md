...
> **偏离说明 (2025-03-18):** PER-01 已放宽。原要求"零动态分配"改为"最小化动态分配"。
> Buffer 使用 `std::string` 内部存储，在可用性和性能间取得平衡。
> 决策记录：STATE.md line 84 - "可用性优先"

- [x] **PER-01**: ~~热路径零动态分配~~ → 最小化动态分配
  - Buffer 使用 std::string 内部存储（预分配 4096 字节）
  - 实际分配仅在大消息时发生（SSO 机制）
  - 可用性优先于极端性能优化
